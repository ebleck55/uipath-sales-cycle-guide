...
class AIChatbot {
  ...
  async processMessage(message) {
    // Always try AI response first if API key is available for better quality
    if (this.hasAICredentials()) {
      // Search local knowledge base to provide context to AI
      const localResults = this.searchKnowledgeBase(message);
      
      // Generate AI response with local context
      return await this.generateAIResponseWithContext(message, localResults);
    }
  ...
  // Unified API key getter - checks both sessionStorage and localStorage
  getApiKey() {
    // First check sessionStorage (user session)
    let apiKey = sessionStorage.getItem('claude_api_key');
    if (apiKey) return apiKey;
    
    // Then check localStorage (admin set key)
    apiKey = localStorage.getItem('claude_api_key');
    if (apiKey) {
      try {
        // Try to decode if it's base64 encoded (from admin)
        return atob(apiKey);
      } catch (e) {
  ...
  // Generate AI response with local knowledge context for better quality
  async generateAIResponseWithContext(message, localResults) {
    const apiKey = this.getApiKey();
    
    // Build context from local knowledge
    let contextInfo = '';
    if (localResults && localResults.length > 0) {
      contextInfo = '\n\nRelevant UiPath Sales Guide information:\n';
      localResults.forEach((result, index) => {
        if (result.question) contextInfo += `${index + 1}. Q: ${result.question}\n`;
        if (result.answer) contextInfo += `   A: ${result.answer}\n`;
        if (result.stage) contextInfo += `   (From: ${result.stage})\n`;
        contextInfo += '\n';
      });
    }
    
    const prompt = `You are an expert UiPath sales consultant. A user is asking: "${message}"

${contextInfo}

Please provide a comprehensive, practical response that:
- Directly answers their question
- Uses your expertise about UiPath's automation platform
- Includes specific, actionable advice
- References the provided guide information when relevant
- Maintains a conversational, helpful tone

Focus on sales processes, objection handling, discovery questions, personas, use cases, and value propositions. If the question is outside sales topics, politely redirect while being helpful.`;
  ...
  // Handle user message from external sources (like search bar)
  handleUserMessage(message) {
    // Open the chat if it's not already open
    this.openChat();
    
    // Set the input field with the message and trigger send
    const chatInput = document.getElementById('chat-input');
    if (chatInput) {
  ...
  // Generate AI response (used by other components)
...
class TimelineUiPathApp {
  ...
  async initialize() {
    ...
    try {
      ...
      // Load admin personas data
      await this.loadAdminPersonas();
  ...
  async loadAdminPersonas() {
    try {
      const response = await fetch('src/data/personas.json');
      if (response.ok) {
        this.adminPersonas = await response.json();
        console.log('Admin personas loaded successfully');
      } else {
        console.warn('Could not load admin personas, using basic personas');
      }
    } catch (error) {
      console.warn('Error loading admin personas:', error);
    }
  }
  ...
  renderTimeline() {
    ...
    if (!container || !SALES_CYCLE_DATA.stages) {
      return;
    }
  ...
  renderPersonas() {
    ...
    // Use admin personas if available, otherwise fall back to SALES_CYCLE_DATA
    let personas = [];
    if (this.adminPersonas && this.adminPersonas.personas && this.adminPersonas.personas[currentIndustry]) {
      personas = this.adminPersonas.personas[currentIndustry];
    } else if (SALES_CYCLE_DATA.personas && SALES_CYCLE_DATA.personas[currentIndustry]) {
      personas = SALES_CYCLE_DATA.personas[currentIndustry];
    }
    
    const personasHTML = personas.map((persona, index) => {
      // Check if this is admin persona with rich data or basic persona
      const isAdminPersona = persona.id && persona.level && persona.influence;
      
      return `
      <div class="persona-card bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow" data-persona-index="${index}">
        <div class="persona-header p-4 cursor-pointer hover:bg-gray-50 transition-colors" onclick="window.TimelineApp.togglePersonaCard(${index})">
          <div class="flex justify-between items-center">
            <div class="flex items-center flex-1">
              <svg class="persona-chevron w-4 h-4 mr-2 text-blue-600 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
              </svg>
                ...
                ${isAdminPersona ? `
                  <div class="flex items-center space-x-2 mt-1">
                    <span class="px-2 py-1 text-xs font-medium rounded-full ${
                      persona.level === 'c-suite' ? 'bg-purple-100 text-purple-800' :
                      persona.level === 'executive' ? 'bg-blue-100 text-blue-800' :
                      persona.level === 'director' ? 'bg-green-100 text-green-800' :
                      'bg-gray-100 text-gray-800'
                    }">
                      ${persona.level === 'c-suite' ? '👑 C-Suite' : 
                        persona.level === 'executive' ? '🎯 Executive' : 
                        persona.level === 'director' ? '📋 Director' : 
                        persona.level}
                    </span>
                    <span class="px-2 py-1 text-xs font-medium rounded-full ${
                      persona.influence === 'decision-maker' ? 'bg-red-100 text-red-800' :
                      persona.influence === 'influencer' ? 'bg-orange-100 text-orange-800' :
                      'bg-gray-100 text-gray-800'
                    }">
                      ${persona.influence === 'decision-maker' ? '✅ Decision Maker' : 
                        persona.influence === 'influencer' ? '🤝 Influencer' : 
                        persona.influence}
                    </span>
                    ${persona.priority === 'high' ? '<span class="px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800">⭐ High Priority</span>' : ''}
                  </div>
                ` : ''}
              </div>
        ...
        <div class="persona-content hidden px-4 pb-4" data-persona-content="${index}">
          <div class="space-y-4 pt-4 border-t border-gray-100">
            
            ${isAdminPersona && persona.lob && persona.lob.length > 0 && !persona.lob.includes('all') ? `
            <!-- Line of Business -->
            <div class="bg-gray-50 rounded-lg p-3">
              <h4 class="text-xs font-semibold text-gray-700 mb-2">Lines of Business</h4>
              <div class="flex flex-wrap gap-1">
                ${persona.lob.map(lob => `<span class="px-2 py-1 text-xs bg-gray-200 text-gray-700 rounded">${lob}</span>`).join('')}
              </div>
            </div>
            ` : ''}
            ...
            ${isAdminPersona && persona.tags && persona.tags.primary ? `
            <!-- Key Focus Areas -->
            <div class="bg-indigo-50 rounded-lg p-4">
              <h4 class="text-sm font-semibold text-indigo-800 mb-3 flex items-center">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="flex flex-wrap gap-2">
                ${persona.tags.primary.map(tag => `<span class="px-2 py-1 text-xs bg-indigo-200 text-indigo-800 rounded-full">${tag}</span>`).join('')}
              </div>
            </div>
            ` : ''}
  ...
  togglePersonaCard(index) {
    console.log('Toggling persona card:', index);
    const content = document.querySelector(`[data-persona-content="${index}"]`);
    const chevron = document.querySelector(`[data-persona-index="${index}"] .persona-chevron`);
    
    if (content && chevron) {
      if (content.classList.contains('hidden')) {
        content.classList.remove('hidden');
        chevron.style.transform = 'rotate(90deg)';
        console.log('Opened persona card:', index);
      } else {
        content.classList.add('hidden');
        chevron.style.transform = 'rotate(0deg)';
        console.log('Closed persona card:', index);
      }
    } else {
      console.error('Could not find persona content or chevron for index:', index);
    }
  }

  attachCollapsibleEventListeners(container) {
    // Handle collapsible functionality for persona cards
    const headers = container.querySelectorAll('.collapsible-header');
    headers.forEach(header => {
      header.addEventListener('click', () => {
        const content = header.nextElementSibling;
  ...
  async handleAIResponse(button) {
    console.log('Discovery AI - handleAIResponse called');
    const question = decodeURIComponent(button.dataset.question);
    const noteId = button.dataset.noteId;
    const textarea = $(`[data-note-id="${noteId}"]`);
    
    console.log('Discovery AI - Question:', question);
    console.log('Discovery AI - Note ID:', noteId);
    console.log('Discovery AI - Textarea found:', !!textarea);
    ...
    if (!this.isAIAvailable()) {
      console.log('Discovery AI - No API key available');
      this.showError('API key not found. Please set your Claude API key in the Admin panel (⚙️ Admin button in top-right corner).');
      return;
    }
    
    console.log('Discovery AI - API key available, proceeding with generation');
    ...
    try {
      ...
      console.log('Discovery AI - Context:', context);
      const response = await this.generateAIResponse(question, context);
      console.log('Discovery AI - Response received:', response?.substring(0, 100) + '...');
      
      // Create or update AI response display box
      this.displayAIResponseBox(noteId, response, 'Discovery Question');
  ...
  getApiKey() {
    ...
    let apiKey = sessionStorage.getItem('claude_api_key');
    if (apiKey) {
      console.log('Discovery AI - Found API key in sessionStorage');
      return apiKey;
    }
    ...
    apiKey = localStorage.getItem('claude_api_key');
    if (apiKey) {
      console.log('Discovery AI - Found API key in localStorage, raw key:', apiKey.substring(0, 20) + '...');
      
      // Admin stores the key directly without encoding, so just return it
      if (apiKey.startsWith('sk-ant-')) {
        console.log('Discovery AI - Using direct API key from admin');
        return apiKey;
      }
      
      // If it doesn't start with sk-ant-, try to decode (legacy support)
      try {
        const decoded = atob(apiKey);
        console.log('Discovery AI - Decoded API key from localStorage');
        return decoded;
      } catch (e) {
        console.log('Discovery AI - Failed to decode, using as-is');
        return apiKey;
      }
    }
    
    console.log('Discovery AI - No API key found in either storage');
    return null;
  }
  ...
  isAIAvailable() {
    ...
    const apiKey = this.getApiKey();
    console.log('Discovery AI - API key check:', apiKey ? 'Found' : 'Not found', apiKey?.substring(0, 20) + '...');
...
Keep your response concise and actionable (2-3 paragraphs maximum).`;
  ...
  async handleAIObjectionResponse(button) {
    ...
    try {
      ...
      // Create or update AI response display box
      this.displayAIResponseBox(noteId, response, 'Objection Handling');
  ...
  // Display AI response in a dedicated, readable text box
  displayAIResponseBox(noteId, response, type) {
    const textarea = document.querySelector(`[data-note-id="${noteId}"]`);
    if (!textarea) return;
    
    // Find or create the AI response container
    let responseContainer = document.getElementById(`ai-response-${noteId}`);
    
    if (!responseContainer) {
      // Create new AI response container
      responseContainer = document.createElement('div');
      responseContainer.id = `ai-response-${noteId}`;
      responseContainer.className = 'mt-3 border-l-4 border-orange-500 bg-orange-50 rounded-lg overflow-hidden';
      
      // Insert after the textarea's parent container
      const insertAfter = textarea.closest('.space-y-3') || textarea.parentElement;
      insertAfter.parentElement.insertBefore(responseContainer, insertAfter.nextSibling);
    }
    
    // Format the response content
    const formattedResponse = this.formatResponseContent(response);
      ...
      <div class="px-4 py-3">
        <div class="flex justify-between items-start mb-2">
          <div class="flex items-center">
            ...
            <h4 class="text-sm font-semibold text-orange-800">AI ${type} Response</h4>
          </div>
          <div class="flex space-x-2">
            <button onclick="this.closest('.mt-3').querySelector('.ai-response-content').select(); document.execCommand('copy');" 
                    class="text-xs text-orange-600 hover:text-orange-800 font-medium">
              Copy
            </button>
            <button onclick="this.closest('[id^=\\'ai-response-\\']').style.display='none';" 
        ...
        <div class="bg-white rounded-md border border-orange-200 p-3">
          <textarea class="ai-response-content w-full text-sm text-gray-800 resize-none border-0 bg-transparent focus:outline-none" 
                    rows="${Math.max(4, Math.ceil(response.length / 80))}"
                    readonly>${formattedResponse}</textarea>
        </div>
        
        <div class="mt-2 flex justify-between items-center text-xs text-gray-600">
          <span>Generated at ${new Date().toLocaleTimeString()}</span>
          <button onclick="this.parentElement.parentElement.parentElement.querySelector('textarea[data-note-id]').value += (this.parentElement.parentElement.parentElement.querySelector('textarea[data-note-id]').value ? '\\n\\n--- AI Response ---\\n' : '--- AI Response ---\\n') + this.parentElement.parentElement.querySelector('.ai-response-content').value;" 
...
class LLMPromptBar {
  ...
  initializeEventListeners() {
    ...
    // AI Response Section Event Listeners
    this.initializeAIResponseSection();
  }

  initializeAIResponseSection() {
    const closeBtn = document.getElementById('ai-response-close');
    const collapseBtn = document.getElementById('ai-response-collapse');
  ...
  clearChatHistory() {
    ...
    // Clear the chatbot's conversation history too
    if (window.chatbot && window.chatbot.conversationHistory) {
  ...
  async handlePromptSubmit() {
    ...
    if (window.chatbot && typeof window.chatbot.processMessage === 'function') {
      ...
      // Show the inline AI response section
      this.showAIResponse(prompt);
      
      try {
        // Add user message to conversation history
        if (typeof window.chatbot.conversationHistory !== 'undefined') {
          window.chatbot.conversationHistory.push({
            role: 'user',
            content: prompt,
        ...
        // Process the message (includes knowledge base search + AI response)
        const response = await window.chatbot.processMessage(prompt);
        ...
        console.error('AI response error:', error);
        this.addChatMessage('user', prompt);
        this.addChatMessage('assistant', 'Sorry, I encountered an error while processing your request. Please try again.');
      }
    } else {
  ...
  // Show the AI response section with loading state
  showAIResponse(query) {
    const responseSection = document.getElementById('ai-response-section');
    const queryDisplay = document.getElementById('ai-response-query');
    const loadingIndicator = document.getElementById('ai-loading');
    
    // Set the query
    if (queryDisplay) {
      queryDisplay.textContent = `Current conversation`;
    }
  ...
  addChatMessage(role, content) {
    ...
    // Hide loading when adding messages
    if (loadingIndicator) {
    ...
    const messageContent = this.formatAIResponse(content);
    ...
    messageDiv.innerHTML = `
      <div class="max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${
        ...
        <div class="text-sm ${isUser ? 'text-white' : 'prose prose-sm max-w-none'}">
          ${isUser ? this.escapeHtml(content) : messageContent}
        </div>
  ...
  // Display error in AI response
  displayAIError(errorMessage, query) {
    const loadingIndicator = document.getElementById('ai-loading');
    const responseContent = document.getElementById('ai-response-content');
    ...
    if (responseContent) {
      responseContent.innerHTML = `
        <div class="text-red-600 bg-red-50 border border-red-200 rounded-lg p-3">
  ...
  formatAIResponse(response) {
    // Basic HTML escaping
    let formatted = response.replace(/[<>&"]/g, function(match) {
      return {
        '<': '&lt;',
        '>': '&gt;',
        '&': '&amp;',
